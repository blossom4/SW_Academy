...
let isValid = password => password.length >= 8 // 소괄호 중괄호 return 모두 생략
isValid('asdf')
...
// 표현식
const join = function (numbers, separator) {
	let res = ''
...
// 선언식
function join (numbers, separator) {
	let res = ''
	for (let i = 0; i < numbers.length; i++) {
		const number = numbers[i]
		res += number
...
// 화살표
const celsiusToFahrenheit = cesius => celsius * 9/5 + 32